...
# ======================================================
# MODULE: GLOBAL BCCI CONSTRUCTION AND VALIDATION
# ======================================================

"""
This module constructs a hierarchical global Brain–Cardiac Coupling Index (Global_BCCI)
from domain-specific BCCI scores and evaluates whether the integrated score
predicts cardiac physiology.

Assumes the dataframe `df_z` already contains the domain-level BCCI scores:
    - BCCI_rate_control_cv_z
    - BCCI_systolic_function_cv_z
    - BCCI_diastolic_function_cv_z
...
Outputs
-------
Global_BCCI_loadings.tsv
Global_BCCI_validation.tsv
Updated BCCI_subject_scores.tsv
"""

print("
============================")
print("Constructing GLOBAL BCCI")
print("============================")

# ------------------------------------------------------
# 1. Select available domain BCCI inputs
# ------------------------------------------------------
...
global_bcci_inputs = [c for c in global_bcci_inputs if c in df_z.columns]

print("Domain inputs used for Global BCCI:", global_bcci_inputs)

if len(global_bcci_inputs) < 2:
    print("Not enough domain BCCI scores to construct global axis")
    ...
    tmp = df_z[global_bcci_inputs].dropna().copy()

    print("Subjects used for global BCCI:", len(tmp))
    ...
    global_scores = pca_global.fit_transform(tmp).ravel()

    df_z.loc[tmp.index, "Global_BCCI"] = global_scores

    # standardize
    df_z["Global_BCCI_z"] = (
        df_z["Global_BCCI"] - df_z["Global_BCCI"].mean()
    ) / df_z["Global_BCCI"].std()

    print("Variance explained by Global BCCI:",
          round(pca_global.explained_variance_ratio_[0], 3))
    ...
    loadings = pd.Series(
        ...
        index=global_bcci_inputs,
        name="Loading"
    )

    loadings.to_csv(
        os.path.join(OUTDIR, "Global_BCCI_loadings.tsv"),
        sep="	",
        header=True
    )

    print("Global BCCI loadings:")
    print(loadings)

    # --------------------------------------------------
    # 5. Optional sign alignment
    # --------------------------------------------------

    ref_var = "BCCI_systolic_function_cv_z"

    if ref_var in tmp.columns:

        corr = np.corrcoef(
            df_z.loc[tmp.index, "Global_BCCI"],
            df_z.loc[tmp.index, ref_var]
        )[0,1]

        if np.isfinite(corr) and corr < 0:

            print("Flipping Global BCCI sign for interpretability")

            df_z.loc[tmp.index, "Global_BCCI"] *= -1
            df_z["Global_BCCI_z"] = (
                df_z["Global_BCCI"] - df_z["Global_BCCI"].mean()
            ) / df_z["Global_BCCI"].std()
...
# ======================================================
# MODULE: GLOBAL BCCI VALIDATION
# ======================================================

print("
============================")
print("Validating GLOBAL BCCI")
print("============================")

validation_rows = []

if "Global_BCCI_z" in df_z.columns:
    ...
    for outcome in outcomes:
        ...
        if outcome not in df_z.columns:
            continue

        tmp = df_z[["Global_BCCI_z", outcome]].dropna().copy()

        if len(tmp) < 10:
            continue

        model = smf.ols(
            f"{outcome} ~ Global_BCCI_z",
            data=tmp
        ).fit()
        ...
        validation_rows.append({
            "Outcome": outcome,
            "N": len(tmp),
            "Beta": model.params.get("Global_BCCI_z", np.nan),
            "P": model.pvalues.get("Global_BCCI_z", np.nan),
            "R2": model.rsquared
        })
...
validation_df.to_csv(
    os.path.join(OUTDIR, "Global_BCCI_validation.tsv"),
    sep="	",
    index=False
)

print("
Global BCCI validation results")
print(validation_df)
