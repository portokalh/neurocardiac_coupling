...
- metric/domain heatmaps
- circuit maps per metric and per domain
- moderation results
- genotype-specific maps
- Figure 4 style summary panels
"""
...
OUTDIR = os.path.join(
    BASE,
    "results/network_cardiac_optimized"
)
...
domain_vars = [d + "_z" for d in CARDIAC_GROUPS.keys() if d + "_z" in df_z.columns]
...
for domain_var in domain_vars:
    ...
    if len(subdf) < max(15, len(network_cols) + 2):
...
covars_for_bcci = [c for c in ["Age", "Sex_Male", "Mass"] if c in df_z.columns]
...
# Network importance by metric / domain
...
# ======================================================
# 16. PLS LATENT BRAIN–HEART MODEL
# ======================================================

print("Running PLS model")

if "Heart_Rate" in df_z.columns:
    X_pls = df_z[network_cols].values
    y_pls = pd.to_numeric(df_z["Heart_Rate"], errors="coerce").values.reshape(-1, 1)

    valid_pls = np.isfinite(y_pls).ravel()
    X_pls_valid = X_pls[valid_pls]
    y_pls_valid = y_pls[valid_pls]

    pls = PLSRegression(n_components=1)
    pls.fit(X_pls_valid, y_pls_valid)
    pred_pls = pls.predict(X_pls_valid)

    print("PLS R2:", r2_score(y_pls_valid, pred_pls))

    kf_pls = KFold(n_splits=N_SPLITS_CV, shuffle=True, random_state=1)
    pred_cv = np.zeros(len(y_pls_valid))
    ...
    for train, test in kf_pls.split(X_pls_valid):
        ...
        pred_cv[test] = pls_cv.predict(X_pls_valid[test]).ravel()

    print("Cross-validated PLS R2:", r2_score(y_pls_valid, pred_cv))
...
# keep legacy-style overall metric association table too
bcci_metric_results = []
...
target_models = [
    ("systolic_function_z", "BCCI_systolic_function_cv_z", "E4_Genotype"),
...
for metric, predictor, mod in target_models:
    if metric not in df_z.columns:
        continue
    if predictor not in df_z.columns:
        continue
    if mod not in df_z.columns:
        continue
...
if BRAINMAPS_ENABLED and "E4_Genotype" in df_z.columns:
    ...
    # Example on rate control domain via univariate network association
    maps = {}
...
# Panel A: rate-control BCCI vs heart rate
...
# Panel C: domain effects with matching BCCI
domain_beta = {}
domain_p = {}
domain_r2 = {}
domain_ci = {}
